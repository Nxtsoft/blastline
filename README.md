...
| **What each commit did** — Commit · Intent · Files · Reaches · Ran before push, folded after 10 commits | each commit's `Entire-Checkpoint:` trailer, resolved to `refs/entire/checkpoints/<shard>/<id>`; without one, the commit's own marks (an `Agent-Logs-Url:` trailer, a vendor address as author or `Co-authored-by:`) name the agent |
