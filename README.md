...
| **Summary** — Changed, Tests reached, Downstream code (with a **review effort** tier: `high` from 20 dependents or 10 mapped files, `low` under 5 dependents and 4 files, `medium` between; fixed thresholds), plus **Intent** (how many commits carry a checkpoint, which models), **Reviewed by** (who has reviewed other than the author, or `no reviewer other than the author (login) so far`; and the humans whose commits last changed the changed and reached files before this range, agent-authored commits set aside), **Symbols** (changed / added / removed / moved), **Since push `<sha>`** (new commits, changed files, tests reached, newly reached files) | the checkpoint refs, `cgraph change-context`, the snapshot embedded in the previous comment |
| **What each commit did** — Commit · Intent · Files · Reaches · Ran before push, folded after 10 commits | each commit's `Entire-Checkpoint:` trailer, resolved to `refs/entire/checkpoints/<shard>/<id>` (a 26-character id) or to `<first two>/<rest>/` on the branch `entire/checkpoints/v1` (a 12-character id); the Intent cell is what the agent said in the turn that produced the commit, then the agent and model; without a checkpoint, the commit's own marks (an `Agent-Logs-Url:` trailer, a vendor address as author or `Co-authored-by:`) name the agent |
| **Claims checked** — `refuted`, `partial` or `consistent`, refuted first | `blastline check` on removed symbols against the base graph; a changed declaration against its base-graph callers outside the diff; the reaching tests against the test commands the checkpoint ran; files touched against the commit; the PR body and each commit message against the diff |
| **What each changed file reaches** — a **Why** column (present when a checkpoint carries one) gives, per file, the agent's own line before the edit that touched each changed symbol, with its turn; a **Read** column gives the order (widest reach first; a changed file another changed file reaches sits right after it as `with <file>`; test files last), the Change column names what happened to each symbol | the reach walk; `cgraph change-context`, `changes[].symbol_changes` |
...
**Without a checkpoint, the commit's own marks still say who.** GitHub's Copilot cloud agent writes an `Agent-Logs-Url:` trailer on every commit; Claude Code and Copilot leave their vendor noreply address (`noreply@anthropic.com`, `copilot@users.noreply.github.com`, matched after dropping GitHub's `<id>+` prefix) as the author or a `Co-authored-by:`. The Intent column then reads `copilot by Agent-Logs-Url · session log` or `claude-code by author`, the commit counts as an agent commit, and the footer says `2 attributed by trailer`. A mark carries no prompt, no files and no test commands, so it makes no claim the brief could check. The footer counts the commits with neither and says what the run lacked: no trailer, no vendor address, and no session index (`--local`) on this machine. A commit whose `Entire-Checkpoint:` ref was not pushed still reads `not fetched`, which names the fix.

**The other open PRs.** The Action reads the newest 30 open PRs' brief comments (`--others <file>` locally, `[{number, body}]`; `others` over MCP) and a **Concurrent PRs** row names each PR whose brief meets this one: what it changes that this PR's code reaches (with the changed symbols named), files both change, and what this PR changes that its code reaches. Two agents on one repository otherwise learn of each other at merge time; on constructed tasks, one line naming the concurrent change recovered 82% of interfering runs (Xia, Wu, Park 2026), and co-active agent PR pairs are most agent PRs (Xu, Subramanian, Karthik 2026). The snapshot every brief embeds carries its changed files and symbols from 0.14.5 on; an older brief cannot be met and is skipped.

**Who has looked, and who knows the reached code.** The Action passes the PR author and its reviews (`--author <login>`, `--reviews <file>` locally; `author`, `reviews` over MCP). The Reviewed-by row names every reviewer other than the author with their latest state, or says `no reviewer other than the author (login) so far`: on public GitHub, 40.1% of agent-authored PRs are examined only by the developer who invoked the agent, against 21.5% of human PRs (Agarwal, Miller, Kastner, Vasilescu 2026), and the row states that fact without deciding whether the author's own reading counts. Beside it, `src/owners.ts` reads the base's history for the changed and reached files in one `git log` and names the humans with the most commits there, so a reviewer who holds a mental model of the reached code is one name away; commits by an agent (a vendor address, an `Agent-Logs-Url:` or `Entire-Checkpoint:` trailer, a vendor co-author) are set aside and counted. When the range has human-authored commits, the row also names the changed and reached files none of those humans has ever committed to (`Taylor has no prior commit in 3 of the 7 files this change touches or reaches: …`): the blind spot DORA's respondents call the blind leading the blind, made visible per PR from history rather than from anyone's confidence. Files the range adds are not counted. Authorship is a proxy for knowing the code, not a measurement of it; history under a file's previous name is not followed. When no reviews were fetched (no token, or the API refused), the row names owners only and the footer says so; it never reads "no reviewer" for "not checked".

**The narrative is a claim too.** The Action passes the PR body (`--narrative <file>` locally; `narrative` over MCP) and the brief reads it with every commit message against the diff, by name: a name set in code font (`` `fetchGraph` ``, `` `src/x.ts` ``) that no changed symbol bears, no changed path matches and no changed line contains is `refuted` as a phantom change; an empty body, a template line ("Describe your changes"), `TODO`/`TBD`/`WIP`, or a `wip`/`fixup!` subject is `refuted` as placeholder text; a file with symbol changes that neither the body nor any message names by path, stem or symbol makes the narrative `partial` (understated scope). Fenced code, HTML comments and links are skipped, so a pasted run log names nothing; shas, versions, flags and ranges in code font are not names, and neither are absolute or home paths (`` `/Applications` ``, `` `~/.config` ``) or bare extensions (`` `.pkg` ``), which name a place on a machine or a kind of file. A repo-relative path counts as carried when a changed line contains it (an import, a script argument), not only when the diff touches that file. The check is by name, so a description in other words is not a mismatch. Why: in 23,247 agent PRs, descriptions claiming changes the diff does not implement were the largest class of mismatch, and mismatched PRs were accepted at 28% against 80% (Gong et al., MSR'26).

**A changed declaration** (`changed` in change-context, with its declaring line both removed and added, differently) whose base-graph callers include files this diff does not touch is `partial`: the call sites may be compatible, so the reviewer checks them; it is never refuted.

The brief **refutes, it never certifies**: `refuted` is authoritative (a removed symbol still has a static caller in a file the diff did not touch; a commit's reaching tests never ran; the agent touched a file that is not in the commit; the narrative names something the diff does not carry), `consistent` means the graph found nothing against the claim, and the word "verified" does not appear. Without checkpoint refs the comment says `no checkpoints on this branch`; without change-context the footer says so; the reach numbers are the same either way.

**Where intent comes from.** [Entire](https://github.com/entireio/cli) (MIT) records one checkpoint per agent commit: the prompts of the turn, the compact transcript, the files touched. Entire 0.10.0 and later give each checkpoint its own ref under `refs/entire/checkpoints/` with a 26-character id; a repository enabled before 0.10.0 keeps every checkpoint on the shared branch `entire/checkpoints/v1` with a 12-character id, until `entire doctor migrate-checkpoints`. Both travel with the feature branch when the agent machine pushes, and the Action fetches both; the brief reads whichever the trailer names. On the agent machine, once per repository:
...
**What the Intent cell says.** The branch backend snapshots one cumulative transcript per session at each checkpoint, and each snapshot is a prefix of the next, so a commit's part of it is the lines after the previous checkpoint of the same session in the range (`sessions[].lines` and `hash` on each checkpoint carry the count forward; the skip happens only when this transcript's first lines hash to the previous snapshot, so a writer that stores one transcript per commit, such as `blastline checkpoint write`, is read whole). `windowsOf` in `src/checkpoint.ts` starts the cell's window at the later of that and the last prompt a person typed before the first tool call that works on one of the commit's files: an `Edit`, `Write` or `MultiEdit` by path, or a `Bash` command that names the file's basename (an agent that edits through a script never calls an edit tool; a common basename can start the window a turn early). The cell shows the first line of the agent's first text in that window and, when it differs, its last (`then: …`). The test commands, and so the reaching tests that ran, come from that whole turn, whatever an earlier commit read of it: a turn that runs the tests once and then makes three commits ran them for all three. A commit whose window holds nothing was made by the same step as an earlier one and says `same step as <sha>`. A commit whose agent said nothing shows the first line a person typed in `0/prompt.txt`; a prompt the harness delivered (a block starting with `<`, such as `<task-notification>`, or a skill's instructions, which start "Base directory for this skill:") is never shown as intent. A skill's later sections split by its own `---` rules, and a slash command's expanded body, carry no mark and are not told apart.

**Privacy allowlist.** `src/checkpoint.ts` reads exactly these fields and nothing else: the checkpoint id, the commit, the agent and model, each session's id and transcript line count, the **first line a person typed** in `0/prompt.txt` capped at 200 characters, the **first and last line the agent wrote** in the commit's window of the compact transcript, each capped at 200 characters, with the count of its text blocks and tool calls, `files_touched`, the `Bash` commands in the compact transcript that name a test runner (`vitest`, `jest`, `pytest`, `go test`, `cargo test`, `ctest`, `gradle test`, `mvn test`, `npm test` and kin), the writer's `source`, and, for each changed symbol the brief already knows, the **first line of the agent's last text before the edit that touched it** (an `Edit`, `Write` or `MultiEdit` on the symbol's file whose text names the symbol; a `Write` touches every symbol in the file), capped at 200 characters. The edit's path is compared and its contents are searched for the name; neither is shown. The raw transcript (`0/full.jsonl`), tool results, file contents and any later prompt line never leave the ref. The allowlist is code, not a flag.
...
Locally, `blastline brief main..HEAD --change-context <file> [--previous <old-comment.md>] [--narrative <pr-body.md>] [--author <login> --reviews <reviews.json>] [--others <briefs.json>] [--json]` renders the same brief before you push, and `blastline_brief` over MCP gives an agent its own brief (`{brief, markdown, check_run}`). `--json` carries the Checks API body the Action POSTs; annotations are capped at 50 per request, GitHub's ceiling.

**In the Action**, the brief is on by default: `cgraph-version` defaults to `bin-v0.5.0` (repo-relative node ids; `bin-v0.4.0` is the first release with `change-context`), and `change-context` runs over the range's diff against a detached worktree of the base (`fetch-depth: 0`); the checkpoint refs and the checkpoint branch are fetched if present; `check-run: "false"` turns the check run off. The token needs one more permission for it:
