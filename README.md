...
**What the Intent cell says.** The branch backend snapshots one cumulative transcript per session at each checkpoint, so a commit's part of it has to be found: `windowsOf` in `src/checkpoint.ts` starts at the later of the previous checkpoint of the same session in the range and the last prompt a person typed before the first `Edit`, `Write` or `MultiEdit` of one of the commit's files. The cell shows the first line of the agent's first text in that window and, when it differs, its last (`then: …`); the test commands and the reaching tests that ran come from the same window. A commit whose window holds nothing was made by the same step as the previous one and says `same step as <sha>`. A commit whose agent said nothing shows the first line a person typed in `0/prompt.txt`; a prompt the harness delivered (a `<task-notification>` or `<system-reminder>` block) is never shown as intent.

**Privacy allowlist.** `src/checkpoint.ts` reads exactly these fields and nothing else: the checkpoint id, the commit, the agent, model, session id and creation time, the **first line a person typed** in `0/prompt.txt` capped at 200 characters, the **first and last line the agent wrote** in the commit's window of the compact transcript, each capped at 200 characters, with the count of its text blocks and tool calls, `files_touched`, the `Bash` commands in the compact transcript that name a test runner (`vitest`, `jest`, `pytest`, `go test`, `cargo test`, `ctest`, `gradle test`, `mvn test`, `npm test` and kin), the writer's `source`, and, for each changed symbol the brief already knows, the **first line of the agent's last text before the edit that touched it** (an `Edit`, `Write` or `MultiEdit` on the symbol's file whose text names the symbol; a `Write` touches every symbol in the file), capped at 200 characters. The edit's path is compared and its contents are searched for the name; neither is shown. The raw transcript (`0/full.jsonl`), tool results, file contents and any later prompt line never leave the ref. The allowlist is code, not a flag.
