...
| **What each changed file reaches** — a **Read** column gives the order (widest reach first; a changed file another changed file reaches sits right after it as `with <file>`; test files last), the Change column names what happened to each symbol | the reach walk; `cgraph change-context`, `changes[].symbol_changes` |
