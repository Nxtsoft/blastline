...
Locally, `blastline brief main..HEAD --change-context <file> [--previous <old-comment.md>] [--narrative <pr-body.md>] [--author <login> --reviews <reviews.json>] [--json]` renders the same brief before you push, and `blastline_brief` over MCP gives an agent its own brief (`{brief, markdown, check_run}`). `--json` carries the Checks API body the Action POSTs; annotations are capped at 50 per request, GitHub's ceiling.
