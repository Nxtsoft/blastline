...
describe("select", () => {
  ...
  it("fails open to ALL when the diff touches an unmapped file", () => {
    const sel = select(DIFF_WITH_DOC, { graph: g });
  ...
  it("an ignore rule rescues the selection for declared-irrelevant files", () => {
    const sel = select(DIFF_WITH_DOC, { graph: g, ignore: (p) => p.endsWith(".md") });
    expect(sel.kind).toBe("subset");
  });

  it("fails open when the graph predates the head commit", () => {
    const sel = select(DIFF_IN_PARSE, { graph: g, graphMtimeMs: 1000, headCommitMs: 2000 });
  ...
  it("fails open when the diff exceeds the file budget", () => {
    const sel = select(DIFF_WITH_DOC, { graph: g, maxFiles: 1 });
    expect(sel.kind).toBe("all");
    if (sel.kind !== "all") return;
  ...
  it("is deterministic: identical inputs produce identical output", () => {
    const a = JSON.stringify(select(DIFF_IN_PARSE, { graph: g }));
    const b = JSON.stringify(select(DIFF_IN_PARSE, { graph: g }));
    expect(a).toBe(b);
  });
});
